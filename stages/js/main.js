...
function nodeActive(a) {
    ...
    d = "";
		for (g in e) {
			...
			f.push('<li class="membership"><a href="#' + c.name + '" '+getBranche(c)+' onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + c.id + '\'])\" onclick=\"nodeActive(\'' + c.id + '\')" onmouseout="sigInst.refresh()">' + c.name + "</a></li>");
			console.log(c);
		}
		return f;
	}
...
function getBranche(c) {
    var nom = c.name;
    
	var patt = /^m[ée]canique[s]?/i
	var patt1 = /^G[S]?M/
	var patt2 = /informatique/i
	var patt22 = /^GI/
	var patt3 = / biologique/i
	var patt33 = /^GB/
	var patt4 = / urbains/i
	var patt44 = /^GSU/
	var patt5 = / Proc[ée]d[ée][s]?/i
	var patt55 = /^GP/
    
    if (patt.test(nom) || patt1.test(nom)) {
        return 'style="color: red"'
    }
    else if (patt2.test(nom) || patt22.test(nom)) {
        return 'style="color: DarkOrange"'
	}
    else if (patt3.test(nom) || patt33.test(nom)) {
        return 'style="color: Fuchsia"'
	}
    else if (patt4.test(nom) || patt44.test(nom)) {
        return 'style="color: black"'
	}
    else if (patt5.test(nom) || patt55.test(nom))  {
